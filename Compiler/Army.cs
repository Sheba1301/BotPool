...
class realmsmc : ChatBot
{
    ...
    public override void GetText(string text, string username)
    {
        string message  = "";
...
public static string GetPassword(string nick, List<MixedRecord> records)
{
    ...
    private void SwitchClickSlots(int g)
{
    // Проверка диапазона
    if (g < 1 || g > 80)
    {
        LogToConsole($"[Switch] Ошибка: номер грифа {g} вне диапазона 1-80.");
        return;
    }

    // Определяем страницу и индекс внутри страницы
    int page = (g - 1) / 32;          // 0, 1 или 2
    int index = g - page * 32;        // 1..32 (на третьей странице 1..16)

    // Переход на нужную страницу (клики по слоту 45)
    for (int i = 0; i < page; i++)
    {
        LogToConsole($"[Switch] Переход на страницу {i+1}, клик по слоту 45");
        ClickSlot(45);
        Thread.Sleep(300); // небольшая задержка между кликами
    }

    // Вычисляем слот на текущей странице по индексу (1..32)
    int slot = ComputeSlot(index);
    
    LogToConsole($"[Switch] Гриф {g} → страница {page+1}, индекс {index}, слот {slot}");
    ClickSlot(slot);
}

// Вспомогательная функция для вычисления слота по индексу (1..32)
private int ComputeSlot(int n)
{
    if (n <= 4)
        return n - 1;           // 1-4 → 0-3
    else if (n <= 12)
        return n;               // 5-12 → 5-12 (пропуск слота 4)
    else if (n <= 20)
        return n + 1;           // 13-20 → 14-21 (пропуск слота 13)
    else if (n <= 28)
        return n + 2;           // 21-28 → 23-30 (пропуск слота 22)
    else
        return n + 3;           // 29-32 → 32-35 (пропуск слота 31)
}
}